...
// Утапливание под нажатием. Заливку при этом не трогаем: цвет уже сменился
// на наведении, и второе затемнение поверх него читается как грязь, а не как
// отклик. Нажатие кнопки говорит геометрией, цветом говорят строки списков.
//
// В базу класса не идёт: единственный вариант без утапливания — `link`, и
// гасить его пришлось бы встречным правилом того же веса, где исход решает
// порядок в собранном CSS, а не намерение. Проверено — там побеждала база, и
// ссылка утапливалась вопреки правилу. Поэтому отклик раздаётся поимённо.
//
  ...
  ],
  {
    variants: {
      variant: {
        ...
        // Наведение — отдельный уровень шкалы, а не доля непрозрачности
        // базовой заливки: такая доля смешивается с подложкой и на светлой
        // поверхности осветляла кнопку вместо того, чтобы её выделить.
        secondary: [
