...
8. **Geist Mono семантический, а не декоративный.** Четыре роли, и только они: `Eyebrow` (микро-лейбл над секцией/карточкой/шапкой таблицы), `Num` (любое число-данные — деньги, счётчики, даты, проценты), лейбл группы в сайдбаре, подпись под значением в `StatCaption`. Всё остальное — Geist Sans. Прямой `font-mono` в прикладном коде — признак того, что нужен `Eyebrow`, `Num` или `StatCaption`.
